...
const prompts = {
  ...
  'Пост для Telegram': {
    ...
    getUserMessage: (content) => `Создай пост для Telegram на основе этой статьи. Пост должен быть привлекательным, информативным, с эмодзи и хештегами.\n\nСтатья:\n${content}`
  }
}
...
export async function POST(request) {
  try {
    ...
    const { actionType, content } = await request.json()
    ...
    const userMessage = promptConfig.getUserMessage(truncatedContent)
